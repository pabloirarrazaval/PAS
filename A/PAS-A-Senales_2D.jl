...
#> title = "Señales continuas 2D"
#> date = "2022-07-24"
#> tags = ["Señales y sistemas"]
#> description = "Pluto Notebooks"
...
# ╔═╡ 40df40fc-b239-4b16-a04a-25e3163a9d01
md"""
!!! alert "Concepto de señal"
	Tal como en el caso de las [señales continuas en $\mathbb{R}$](SS-A-Senales.jl.html), una señal contiene información y por lo general representan la entrada o salida de algún [sistema](SS-A-Sistemas.jl.html) o proceso físico. En el caso de las señales en dos dimensiones, la diferencia es que la amplitud de la señal depende de dos variables independientes, lo que implica que son las entradas o salidas de [sistemas de dos dimensiones](PAS-A-Sistemas_2D.jl.html), tal como se define a continuación.
"""
...
Una función puede pertenecer a uno o más [espacios de funciones](SS-A-Funciones_espacios_y_convergencia.jl.html) $L_p(a,b,c,d)$ donde $a,b,c,d$ son los límites del intervalo donde la señal es considerada y $1 \leq p \lt \infty$. Para pertencer a un espacio en particular, la integral

!!! note "Espacios y convergencia"
...
!!! warning "Funciones integrables o cuadrado integrables"
...
!!! note "Señal de energía"
	$$E_f = \int_{-\infty}^{\infty} \int_{-\infty}^{\infty} |f(x,y)|^{2} dxdy$$

	Si esta integral converge, lo que implica que la energía es finita, entonces la 
	señal se considera una **señal de energía**.
...
begin
	...
	surface(x, y, z, xlabel="x",ylabel="y",c=:inferno,nx=80, ny=80,display_option=Plots.GR.OPTION_SHADED_MESH,margin=5mm)
end
...
begin
	...
	heatmap(x,y,z,c=:grays, legend=false, axis=nothing,xlabel="x",ylabel="y",margin=5mm)
end
...
md"""
!!! warning "Operaciones sobre señales separables"
	La operación sobre señales separables es mucho más simple que para el caso general de señales no-separables y nos permite realizar operaciones como la [convolución](PAS-A-Convolucion_2D.jl.html) y la [Transformada de Fourier](PAS-A-Fourier_2D.jl.html) de forma simplificada.
"""
...
!!! note "Señal periódica"
	$f(x,y) = f(x+T_1,y) = f(x,y+T_2)$

!!! warning "Ejemplo"
	Un ejemplo de una señal 2D periódica es la siguiente:

	$$f(x,y) = \cos(2 \pi x) \cos(4 \pi y)$$

	cuyo gráfico se muestra a continuación, como una superficie y como una imagen.
"""
...
begin
  ...
  nx=50, ny=50, display_option=Plots.GR.OPTION_SHADED_MESH, xlabel="x",ylabel="y",margin=5mm)
	pper2 = heatmap(-2:0.05:2, -2:0.05:2, per, c=:grays, axis=nothing,xlabel="x",ylabel="y",margin=5mm)
...
!!! note "Simetría par"
	Si $f(-x,-y) = f(x,y) \,\, \forall x,y$

!!! note "Simetría impar"	
	Si $f(-x,-y) = -f(x,y) \,\, \forall x,y$

!!! note "Simetría hermitiana"
	Si $f(-x,-y) = f^{\ast}(x,y) \,\, \forall x,y$

!!! note "Simetría circular"
	Si $f(x,y) = f(s,t)$, cuando $\sqrt{x^2+y^2} = \sqrt{t^2+s^2} \,\,\,\, \forall x,y,s,t$
